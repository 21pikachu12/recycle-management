...
function toggleStatus(id, type) {
    ...
    // Assuming your PHP file is named updateStatus.php
    xhr.open("GET", "updateStatus.php?id=" + id + "&type=" + type, true);

    // Set up the callback function to handle the response
    xhr.onreadystatechange = function () {
        if (xhr.readyState == 4 && xhr.status == 200) {
            // Update the label without reloading the page
            var label = document.querySelector('a[data-id="' + id + '"] label');

            // Toggle the label text and update the 'onclick' attribute
            if (type === 'active') {
                label.innerHTML = 'Inactive';
                label.parentElement.onclick = function () {
